...
module CarrierWave
  module Storage
    class FTP < Abstract
      ...
      def cache_file
        @cache_filename = Rails.root.join(CarrierWave.tmp_path, uploader.cache_path)
        FileUtils.mkdir_p @cache_filename.dirname unless Dir.exist?(@cache_filename.dirname)
        @cache_filename
      end
